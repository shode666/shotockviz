...
- [x] **20:00 ICT overnight-gap Telegram digest, scoped to one user's book** — `bd:shotockviz-06z`, 2026-09-06, not yet committed
  ...
  - ⚠️ Note formerly said BLOCKED pending `bd:shotockviz-4d9` — that bead closed 2026-09-06 (`fde2095`, `telegram_is_dry_run` guard). Observed live 2026-09-07 21:24 ICT: `celery-beat` on this dev stack IS now armed and fired this task for real against the dev DB's real `telegram_chat_id`; the dry-run guard, not a dormant schedule, is what stopped an actual send (`celery-worker` logs: "TELEGRAM DRY RUN — message NOT sent"). No action item from this — the guard held — but the "dormant until restarted" assumption in this note is stale.
- [x] **Gap list named WHY a symbol has no price instead of an undifferentiated count** — `bd:shotockviz-06z.2`, 2026-09-07, not yet committed
  - `classify_excluded_symbols()` in `gap_list_digest.py`: every book symbol not in the results is now named `market_closed` (reuses `alert_symbol_refresher._is_market_open_for`, not reimplemented) or `missed` (market open, still no usable quote — the real pipeline-gap case). Kept `book_count` = watchlist ∪ holdings unchanged (already pinned by tests) rather than scoping the book to US-session symbols only, following `portfolio_service.py` rules 2/5's "excluded, never silently dropped" pattern. 16 new tests, red-proven via `git stash` on the source file. backend 928 passed, 2 skipped.
