        ...
        for ysym in yahoo_syms:
            ...
            try:
                ...
                fundamentals = StockFundamentals(
                    ...
                    # field bd:shotockviz-f14 reserved on
                    # `StockFundamentals` (models/schemas.py) and nothing
                    # wrote until this.
                    #
                    # bd:shotockviz-f14.2 — the other two writers of
                    # `fundamentals:{symbol}` now stamp it too
                    # (`workers/on_demand_listener.py:_fetch_fundamentals`
                    # and `services/cache_orchestrator.py`), so the read
                    # side's TTL-inferred fallback was DELETED rather than
                    # kept and labelled: a guess you can remove beats a
                    # guess you have to explain. `ts` is therefore either a
                    # real stamp or `None` meaning genuinely unknown — it
                    # is never estimated. Keys written before that deploy
                    # carry `ts=None` and self-heal on their next fetch.
                    ts=int(time.time()),
                )
